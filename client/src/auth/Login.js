...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (direction === "UpdateProfile") {
        navigate("/confirmProfileUpdate");
        setSuccessMessage(
          "Login succesful.Soon you will be directed to homepage"
        );alert({succMessage})
      } else if(direction==="ProfileOk") {
        setSuccessMessage(
          "Login succesful.Soon you will be directed to homepage"
        );alert({succMessage})
        navigate("/homepage");
  ...
  async function checkUserDetails() {
    try {
      const response = await axios.get("http://localhost:4040/checkDetails", {
        withCredentials: true,
      });
      //    console.log();
      setDirection(response.data.p_status);
    } catch (error) {
      throw error;
    }
  }
      ...
      <div className="form_container p-5 rounded bg-dark">
        <form action="" onSubmit={handleSubmit}>
          ...
          <p className="text-end mt-2">
            Forgot <Link href="">Password </Link> Dont have an account{" "}
